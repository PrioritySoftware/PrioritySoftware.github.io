...
Starts a Direct Activation. These are procedures or reports that use the data in the currently active row.

| **Param** | **Type** | **Description** |
| --- | --- | --- |
| ename | string| Name of procedure/report. |
| type | string| Type : specify "R" for reports, "P" for procedures |
| progressCallback | [`DirectActProgressCallback`](#DirectActProgressCallback)| Called in case the 'activitation' in question takes some time to complete. Indicates a progress bar should be shown. |
...
This callback is used to update a progress bar, and is only called when the procedure is  defined as one that includes a progress bar.

Note that if the procedure activates a process that doesn't report progress, or is delayed due to another reason (for example, due to a query running for a long time), this callback WOULD NOT be called.

Therefore, we recommended that you always use some sort of action indicator when running a procedure/report. Change it to a percentage indicator when this
callback is called.
...
| --- | --- | --- |
| proc | `Proc`| Procedure Object. |
| error | [`DirectActError`](#DirectActError) | Object. |
