...
Use this function to update a progress bar. This function is called whenever a progress bar is called on the server by the procedure itself.

Note that as long as a progress bar was not called by the procedure, this callback WOULD NOT be called, even if the procedre is taking a long time (e.g., due to a long-running query).
