...
| --- | --- | --- | --- |
| NoHtml | <code>string</code> | noHtml | There was an attempt to add text in a no-text form. |
| ReadOnlyForm | <code>string</code> | readOnlyForm | There was an attempt to save text in a read-only form. |
| TextTooLong | <code>string</code> | textTooLong | There was an attempt to save a text that was too long. |
| MustSaveOrDiscard | <code>string</code> | mustSaveOrDiscard | There was an attempt to perform an action in some forms row  before saving/discarding changed in the current row. |
| Exception | <code>string</code> | exception | There was an API exceptionץ |
| NoClearOnSubForm | <code>string</code> | noClearOnSubForm | There was an attempt to call 'clearRows' on a subform. |
| FileIsNull | <code>string</code> | fileIsNull | The file sent to the upload function was null. |
| NoSuchSubForm | <code>string</code> | noSuchSubForm | There was an attempt to perform an action on  a not-existing subform. |
| NoSuchColumn | <code>string</code> | noSuchColumn | There was an attempt to perform an action on  a not-existing column. |
| FailedPreviousRequest | <code>string</code> | failedPreviousRequest | The previous API request was deleted from the request-queue, meaning it was rejected. |
| WarningCancel | <code>string</code> | warningCancel | The user canceled a warning message that came from Priority. |
| LoginFailed | <code>string</code> | loginFailed | Login failed. |
| InvalidFilter | <code>string</code> | invalidFilter | The filter for rows retrivial is wrong. |
| ReadWrite | <code>string</code> | readWrite | Text forms - the text is already being edited somewhere else. |
| Stop | <code>string</code> | stop | The current API request was stoped. |
...
| **Name** | **Type** | **Value** | **Description** |
| --- | --- | --- | --- |
| Error | <code>string</code> | error | Indicating there was a error message from Priority. |
| Warning | <code>string</code> | warning | Indicating there was a warning message from Priority. |
| APIError | <code>string</code> | apiError | Indicating there was a fatal error somewhere. Could be a syntax error, API error, exception and so on. |
