...
| format | string| In date fields, the field's format, e.g. 'DD/MM/YY'. Date and time data must match the [ISO 8601 datetime format](https://www.w3.org/TR/NOTE-datetime) and use UTC. |
...
This object is obtained through [`UploadFileProgressCallback`](#UploadFileProgressCallback) or through [`UploadFileSuccessCallback`](#UploadFileSuccessCallback). In the case of the former, it shows the progress of the upload; in the case of the latter, it provides the filename on the server (and a progress of 100%).
...
| file | Object| If the upload was successful, the object containing the file's name. **Key:**name, **Value:** filename. |
