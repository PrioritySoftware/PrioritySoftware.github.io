...
{% include alert.html content="<p>Potentially Breaking Change</p>" %}
  ...
  Starting with version 21.0, the timezone can be set per company, which might lead to a different offset than expected. Verify the timezone of the company with the Priority administrator.

  In previous versions, the time offset was based on the timezone of the server.
