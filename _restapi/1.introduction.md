...
**Note:** Up to version 21.0, time offset was based on the timezone of the server. Starting with version 21.0, the timezone can be set per company, which might lead to a different offset than expected. Verify the timezone of the company with the Priority administrator.
