...
<span class="version-highlight">25.0</span>
...
<code>GET serviceRoot/GetMetadataFor(entity='ENTITYNAME')</code>

For example:

Request:

<code>GET serviceRoot/GetMetadataFor(entity='CUSTOMERS')</code>
...
You can only request metadata for top-level entities (i.e., you can request metadata for ORDERS but not for ORDERITEMS).

**Note:** If no requests were made to the entity, the metadata may not be prepared yet. The easiest way to prepare the form is making a request to the entity. We recommend limiting the results to 1 (using $top=1) to get a faster response, i.e. <code>GET serviceRoot/CUSTOMERS?$top=1</code>.

#### Mandatory Columns

<span class="version-highlight">25.1</span>

Starting with version 25.1, the metadata includes an annotation indicating which columns are mandatory. This annotation appears as `Priority.OData.Mandatory` with a value of `true` for required fields.

Example:

```xml
            <EntityType Name="CUSTOMERS">
                <Annotation Term="Priority.OData.Description" String="&#x5DC;&#x5E7;&#x5D5;&#x5D7;&#x5D5;&#x5EA;"/>
                <Property Name="CUSTNAME" Type="Edm.String" MaxLength="16">
                    <Annotation Term="Priority.OData.Description" String="&#x5DE;&#x5E1;. &#x5DC;&#x5E7;&#x5D5;&#x5D7;"/>
                    <Annotation Term="Priority.OData.Mandatory" Bool="true"/>
```

This information is useful when creating or updating entities, as it helps identify which fields must be provided in the request.
