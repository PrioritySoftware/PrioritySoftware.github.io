...
## Documentation Update (March 26)

- Updated [$since query option](../query/#Retrieving_Only_Records_With_Changes) documentation to recommend using UTC timezone format (Z suffix) to avoid daylight saving time issues.

## Documentation Update (December 25)

- Version 25.1 is released 🎉.
- New constant to control number of returned records, MAXAPILINES, separate from MAXFORMLINES.
- API transactions now use a shared count for all users, replacing the per-user transaction limits from previous versions.


## Documentation Update (November 25)

- Updated the Postman collection with the new sandbox environment.
- Postman collection now uses variables for the service root url and credentials.
- Corrected documentation on [requesting metadata for a specific entity](../request/#Requesting_the_Service_Metadata).

## Documentation Update (September 25)

- Added a link to the new sandbox environment. 
- Updated the example in attaching files while providing a suffix to include a period.
